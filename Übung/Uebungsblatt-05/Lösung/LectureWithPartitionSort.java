...
public class LectureWithPartitionSort
{
    private String title;
    private Student[] students;
    private int firstUnused; 

    public LectureWithPartitionSort( String t, int cap ) 
    {
        title = t;
    ...
    /* see also Dijkstra's Dutch national flag problem */

    public void partitionSortBySubject() {
        int firstInf = 0, firstUnsorted = 0, lastUnsorted = firstUnused-1;
        while ( firstUnsorted <= lastUnsorted )
        {
            if ( students[firstUnsorted].getSubject().equals("DS") )
            {
                // subject is "DS"
                swapStudents(firstInf,firstUnsorted);
                firstInf++;
                firstUnsorted++;
            } 
            else if ( students[firstUnsorted].getSubject().equals("Inf") )
            {
                // subject is "Inf"
                firstUnsorted++;
            } 
            ...
            {
                // subject is "M"
                swapStudents(firstUnsorted,lastUnsorted);
                lastUnsorted--;
            }
        }
    }
